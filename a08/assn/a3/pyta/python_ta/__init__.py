...
def _check(
    ...
    messages_config = _load_messages_config(
        messages_config_path, messages_config_default_path)

    global PYLINT_PATCHED
    if not PYLINT_PATCHED:
        # Monkeypatch pylint (override certain methods)
        patch_all(messages_config)
        PYLINT_PATCHED = True
...
def _load_config(linter: PyLinter, config_location: AnyStr, verbose=True) -> None:
    """Load configuration into the linter."""
    _config_initialization(linter, args_list=[],
                           config_file=config_location, verbose_mode=verbose)
    linter.config_file = config_location


def _override_config(linter: PyLinter, config_location: AnyStr, verbose=True) -> None:
    ...
    config_file_parser = _ConfigurationFileParser(
        verbose=verbose, linter=linter)

    try:
        _, config_args = config_file_parser.parse_config_file(
    ...
    try:
        ...
        linter.add_message("unrecognized-option",
                           args=unrecognized_options_message, line=0)
...
def reset_linter(
    ...
    new_checker_options = (
        (
            "pyta-type-check",
            {"default": False, "type": "yn", "metavar": "<yn>",
    ...
    if isinstance(config, str) and config != "":
        ...
        if pylintrc_location:
            set_config(linter, pylintrc_location, verbose)
...
def _verify_pre_check(filepath: AnyStr) -> bool:
    ...
    try:
        ...
        print(
            "[ERROR] python_ta could not check your code due to a " +
